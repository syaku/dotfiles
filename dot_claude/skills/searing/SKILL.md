...
**記述の規律（読みやすさ）**: 各節は結論の文から始め、1 パラグラフ 1 主張。出典（path:line）は文中括弧で書かず節末にまとめる。**Decisions は {質問・回答・含意} を 1 行ずつの表で書く**（質問トレースの長文化が premise の読みにくさの主犯）。Purpose / Acceptance が読む層の先頭なので、別途の要約節は作らない（二重表明はドリフトの温床）。
