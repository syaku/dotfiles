...
- 本体が使うのは Read/Glob/Grep/Write/Workflow/AskUserQuestion と、step 5 の閉じレビュー 1 体に限った Agent のみ。Bash/Edit と、それ以外の Agent は使わない（agent の fan-out は workflow script の領分。閉じレビューは単発の検査器であり、fan-out でも件数集計でもないため例外とする）。
...
質問文が前提として含む事実（配備・リリースの順序・migration の実行系・DB 設定・既存挙動）は、質問より先に出典付きで確定させる。Read/Glob/Grep で確認できるものは確認してから質問し、確認できない運用事実は前提として断定文に書かない（「未検証」と明示して質問するか、分岐ごと Risks の未検証前提に載せる）。workflow の調査結果に `operational_facts` があれば、`verified=true` のものだけを質問の前提に使う。
...
命名・画面文言・データ定義の流儀など、既存コードの流儀・規約・実測から一意に導ける創作的確定は AskUserQuestion に載せず、plan の「既定採用」節に既定値と根拠（出典 path）で列挙されている状態にする（節が無ければ承認後 Write の範囲で足す）。質問に載せるのは、業務仕様の選択・trade-off が拮抗する選好・premise の Open questions 由来の未決など、ユーザにしか決められない分岐だけ。判定基準は「その答えを既存流儀・実測から一意に導けるか」——導けるなら既定にし、導けないなら質問する。

- 失敗接地: 実走で、判定メソッド名・画面語彙・データ定義の流儀を 1 問ずつ質問した。流儀の実測までしておきながら質問に倒しており、「実装方針について細かく聞かれすぎている」というフィードバックを受けた。

## 本体編集の事実規律（全 step 共通）

pipeline の検証保証は、本体が plan を編集し始めた時点で失効する。承認往復の中で本体が plan に書き足す・書き換える実コードや運用についての事実主張は、Read/Glob/Grep で実体を確認したものだけを事実の形で書く。呼び出し関係・経路・網羅性の主張（「全経路が X を通る」「呼び出し元はこの N 箇所」「この層で走る」）は特に、列挙した各項目を実際に読んで確認するまで plan に書かない。確認していない推測を書く場合は推測と明示するか、Risks の未検証前提に置く。

- 失敗接地: 実走で、承認往復中に本体が「保存系メソッドを呼んでいるから版が増える」という推測から更新経路 6 つを plan に書き、うち 3 つが誤りだった。バリデーションを足す層も同名別系統のメソッドを取り違えた。どちらも pipeline の検証段はとうに終わっており、事後に差し込んだ設計レビューまで残存した。

## フロー

### 1. タスクの理解と出力先決定

- 依頼を整理する。曖昧点のうち Read/Glob/Grep で確定する事実は聞かずに調べ、ユーザにしか決められない分岐だけを AskUserQuestion で **1 問ずつ・依存順に**確認する（束ね打ちしない。sear-me の原則と同じ。premise.md がある場合は下記のとおりそもそも再質問しない）。
...
- **起草前の設計分岐の確認**: 依頼文・premise の「plan への申し送り」・Open questions から**この時点で既に見えている**設計判断レコード級の分岐（競合する選択肢がどちらも成立しうる構造判断）は、workflow 起動前に AskUserQuestion で 1 問ずつ・依存順に確認し、回答を step 2 の `request` に織り込む。premise が確定するのは Purpose / Acceptance であって設計分岐はその軸ではないため、「再質問しない」規約には抵触しない。起草中に初めて発見される分岐は事前には聞けない（workflow 内はユーザ対話不可）——それらは「暗黙の設計判断」観点が step 4 のゲートに乗せ、選択の差し替えに至れば step 5 の再実行ルートが受ける。この事前確認は再実行の頻度を下げる補助で、ここで聞き尽くす義務はない（見えていない分岐を推測で列挙しない）。
...
- **既定採用の提示**: plan に「既定採用」節があれば、承認時に一覧として提示し、異議のある項目だけ上書きを受ける。項目ごとの AskUserQuestion に分解しない（質問プロトコルの裁量既定を参照）。

#### 記述矛盾は採否ゲートに出す前に修復する

`findings` のうち、**plan 自身の記述どうしの矛盾**（同一対象への相互排他な言明が plan 内に両在するもの）で、どちらが正かを次のいずれかで一意に決められるものは、採否ゲートに出す前に plan 本文を修復する。(a) 統合ブランチ上の出典（path:line / sha）、(b) plan 内の正本記述との整合（設計判断レコードの行・premise の決定と噛み合う側が一意に決まる場合。コードの出典では決まらない意図の食い違いはこちらで決まることが多い）。修復した指摘は `findings` から消さず、対応済みとして「どちらを正としたか」と根拠（出典、または正本記述の逐語）を併記し、従来どおり全件列挙する。

- **線引き**: 修復とは指摘を解消済みの状態で全件提示すること。消すとは列挙から落とす・件数を減らす・複数を畳むこと。前者は plan への操作、後者は報告への操作で、厳守プロトコルの禁止が縛るのは後者だけ。
- 出典でも正本記述でも勝ち側を決められない矛盾は修復せず、従来どおり要判断としてユーザへ出す（「出典なき自動修正の禁止」と同じ扱い）。
- 内部矛盾は severity の例外ではなく、severity と直交する種別として扱う。改稿 agent の範囲制約（medium/low は触らない）はそのまま生きる。修復するのは本体で、訂正の経路に載せる。
- 失敗接地: pipeline は出典付き自動訂正を検証段だけに、改稿を severity:high だけに限っているため、medium/low の内部矛盾が全部ユーザの採否ゲートに落ちる。実走で、自分が書いた記述の食い違い 4 件を採否として問い、「自身で記述した文言の不一致を設計判断とするのはおかしい」と指摘された。

### 5. 承認後の反映（経路選択）・閉じレビュー・引き継ぎ

採否が確定したら、反映の前に**経路を採否の深さで選ぶ**（当て込み一律にしない）:

1. **premise 矛盾 → sear-me 差し戻し**: 採否が premise の Purpose / Acceptance と矛盾する方向なら、plan 内で反映しない。plan.md は書き換えず、どの採否が premise のどこと矛盾するかを示して sear-me の再実行（premise の更新）をユーザに提案し、スキルを終了する（呼び出し元に差し戻し理由を返す）。Purpose / Acceptance は premise が正本で plan は再定義しない契約——当て込みでも pipeline 再実行でも吸収できない。
2. **設計判断レコードの選択差し替え → pipeline 再実行**: 採用指摘の反映が、Design 節の設計判断レコードに記録された選択を別の選択肢へ差し替えることを含むなら、当て込みにしない。採否結果を `request` に織り込んで step 2 の workflow を再実行し、戻りを step 3〜4 で再度ゲートに乗せる（差し替えは Approach・Phase・Verification に連鎖するため、検証・レビューを通らない当て込み経路で plan に入れない）。再実行は 1 回を上限とし、2 回目が要る状況は前提が揺れているシグナルとして経路 1 を疑う。
3. **それ未満 → 本体の当て込み**: 文言修正・個別 Phase の受入追記など、レコード上の選択が変わらない採用指摘は本体が plan の該当箇所に当てて反映する（**承認後 Write = 全文 Write 固定**）。採用指摘ゼロならこの Write はスキップ。
...
承認後 Write を終えた plan の最終形に対し、次のいずれかに該当する場合は最終確定の前に閉じレビューを 1 回実施する。(a) complexity が heavy、(b) 承認往復で本体が pipeline 出力に無い事実主張・設計記述を plan に書き足した、(c) ユーザが明示的に求めた。いずれにも該当しない軽量 plan ではスキップしてよい。

- **形**: read-only を指示した Agent 1 体。入力は plan.md・premise.md・統合ブランチの実コード。fan-out しない・件数集計を任せない（厳守プロトコルの例外はこの 1 体だけ）。
- **重点順**: (1) pipeline 完了後に本体編集で入った記述の事実照合——呼び出し経路・層の特定・網羅性の主張は列挙された各項目を実読で確認させる、(2) 内部整合——相互排他な言明・設計判断レコードと本文の食い違い、(3) premise との整合。指摘には severity と統合ブランチ上の出典（path:line）を必須とする。
- **戻りの処理**: 指摘は step 4 と同じ規律で扱う（全件列挙・記述矛盾の修復・事実誤認は出典付きで訂正提示・要判断はユーザの採否）。反映後に閉じレビューを再帰させない（1 回限り。再帰させると収束保証が無い）。
- 失敗接地: 実走で、承認往復中に本体が注入した事実誤認 2 件（更新経路の列挙・バリデーション層）を pipeline は検知できず、手動で差し込んだ単一文脈の設計レビューが指摘 5 件として拾った。この段はその手動レビューの標準化。

#### ナレッジ化候補の提示（終了時）

終了前に、`corrections_applied`・覆った `operational_facts`・閉じレビューの事実訂正から、「書き残されていなかったから誤認された」知識（設計イディオム・命名の罠・運用事実）を選び、1 行ずつナレッジ化候補として提示する。**提示のみで Write はしない**（plan.md 以外の Write 禁止は維持）。置き場の判断が要るものは /knowledge-placement の利用を促す。候補ゼロならその旨を 1 行で言って終わる。
...
- **レビュー観点の節依存**: 評価観点は plan の節構成に応じて該当するもののみ評価。Design 節が無い軽量 plan で責務境界を指摘しない・Phase 節が無い plan で順序の可逆性を指摘しない。内部整合・検証十分性・暗黙の設計判断は常時評価。premise.md がある場合のみ目的整合を評価。
- **設計判断レコード**: Design 節に「検討した分岐・選んだ選択肢・捨てた選択肢と理由・判断が依存する事実と出典」を 1 行ずつ記録する。Approach 本文は推奨案のみ（代替案の並記はしない）で、分岐の存在と選択理由はレコード側が担う。レビューの「暗黙の設計判断」観点（常時評価）が、記録なしに片方へ倒された分岐を選択肢の形（a/b/c＋各 trade-off）で指摘し、要判断としてゲートに乗せる（失敗接地: 2026-06-30 harvest 純関数化 plan。設計レベルの分岐 2 件——Phase 同時反映の保証方式・opts override と不変条件の整合——が要判断に乗らず、記述レベルの指摘だけがゲートに届いた。ユーザ指摘「本来は設計上重要な判断が要判断になるはず」）。
- **決定の単一表明**: 一つの決定（データ定義・置き場・リリース単位・例外の扱い等）は設計判断レコードの 1 行を正本とし、他節では結論を再記述せず参照で済ませる。同じ決定を複数節に再記述すると、点的な訂正・改稿が片方だけ直して内部矛盾が生まれる（失敗接地: 実走で、複数節に再記述された決定 4 件が改稿のたびにずれ、全部が要判断ゲートに落ちた）。
- **運用事実の出典化**: 調査 agent は計画の決定に効く運用事実（migration の実行系・配備順序・DB 設定など）を `operational_facts` として返し、repo 内の出典で確認できたものだけ `verified=true` とする。起草は `verified=false` の事実に決定を載せず、載せる場合は Risks に未検証前提として明示する。レビューは「前提の出典」を常時評価する（失敗接地: 実走で、配備順序の誤前提の上に DDL の段取りと Phase 構成が載り、実測で計画が覆った）。
- **既定採用節**: 創作的確定（命名・文言・定義の流儀）は既存流儀から導いて「既定採用」節に出典付きで列挙する。既存流儀・実測から導けない分岐だけが設計判断レコード経由でユーザに届く（質問プロトコルの裁量既定と対になる script 側の出口）。
- **調査の資料先行**: 調査 agent はコードを grep する前にリポジトリ内の設計資料（docs/・README・CLAUDE.md・ADR 等）を当たり、DB スキーマは migration / DDL を一次ソースとする。変更対象データの書き込み経路と履歴・版管理の性質を current_behavior に必ず含め、資料から得た設計イディオム・命名の罠は notes に含める（失敗接地: 実走で、版管理イディオムの境界——どの操作が版を増やすか——をスポット読みからの推測で誤った）。
- **検証の比例性**: Verification は既存の検証手段を優先し、構築コストが対象の変更より大きい検証装置の新設を提案しない。検証十分性レビューは「覆えているか」と「過大でないか」の双方向（失敗接地: 受入条件・assertion の検証のために対象より巨大な仕組みを作ろうとし、ユーザが手動で止めた事例）。
- **TDD 適用宣言**: 変更がテスト可能なロジックを含み Acceptance が自動テストで観測できるなら、Verification 冒頭に「TDD 適用」と Acceptance 由来のテストリスト（1 テスト = 1 振る舞い）を書く。該当しなければ「TDD 不適用」と理由を 1 行書く。implement がこの宣言を読んで `tdd: true` を既定にする——TDD のトリガーをユーザ発話から plan 判断へ移す（失敗接地: implement-pipeline 全 21 起動で tdd 指定 0 回。「依頼が TDD 志向なら」のユーザ発話依存トリガーは発火しなかった）。宣言の有無と理由はレビューの検証十分性観点が見る。
...
- **文体契約**: 起草・改稿 agent は `~/.claude/output-style.md` を Read して地の文を従わせる（workflow agent は main の常時ロード規範を継承しない。失敗接地: 2026-06-30、「runtime 窓」等の即席合成名詞が plan.md に混入しユーザ指摘で全文点検になった）。
- **レビューへの調査結果の受け渡し**: review agent には起草と同じ explore 構造化結果を渡す（plan 文面に無い暗黙の選択・見送られた再利用候補を文脈から指摘可能にする。repo を独自に調べない規約は維持）。
...
- **script の判断系規約（観点インフレ禁止・deferred-by-design・出典なき修正禁止・改稿範囲制約・質的減少停止・節依存レビュー・severity:high の新基準・設計判断レコード・決定の単一表明・運用事実の出典化・既定採用節・調査の資料先行・暗黙の設計判断レンズ・検証の比例性・TDD 適用宣言・コードの深さの天井・文体契約）を prompt から削る**（interface 節参照。これらはモデル能力でなく判断方針の規約で、モデル世代に依らず残す）
- **設計判断レコードの選択差し替えや premise 矛盾を本体の当て込みで反映する**（step 5 の経路選択の迂回。差し替えは pipeline 再実行、premise 矛盾は sear-me 差し戻し。当て込みで許すのはレコード上の選択が変わらない採用指摘だけ）
