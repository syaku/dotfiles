...
- **script の判断系規約（観点インフレ禁止・deferred-by-design・出典なき修正禁止・改稿範囲制約・質的減少停止・節依存レビュー・severity:high の新基準・設計判断レコード・暗黙の設計判断レンズ・検証の比例性・コードの深さの天井・文体契約）を prompt から削る**（interface 節参照。これらはモデル能力でなく判断方針の規約で、モデル世代に依らず残す）
