...
- **家風タイトルの収集**: frontmatter tags に `気づき`/`洞察` を含む notes を `rg -l` で洗い、ファイル名（拡張子なし）の一覧を作る（多い場合は新しめ 20 件程度）。命名 agent が vault の確立した家風を実例として倣うための同梱素材（失敗接地: 2026-06-12 drain 初回実走で命名 agent が素の文脈で命名し、説明文型の長いタイトルを量産した）。
- **運用ログの収集**: `notes/distill運用ログ.md` を Read して全文を取る（args `distill_log_text` で workflow に渡す）。workflow script が ✗→○ 訂正ペアと「初稿 通過」型を regex 抽出し、家風タイトル（rg ピック）より優先して命名 agent に few-shot 注入する（人ゲート approve 済み＝最も家風に合致する教師データ）。
