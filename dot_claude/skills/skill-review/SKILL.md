...
- 既定 basename: `<YYYY-MM-DD>T<HH:mm> skill-review-<対象 skill 名>.md`（**時刻サフィックス常時付与**）。呼び出し時の明示 basename があれば優先。失敗接地: 2026-07-04 監査で、日付だけ／時刻付きの 2 系統が同日同 skill で並走し inbox 上に版違いが 4 件観測された（sear-me 2026-07-02〜03 の 4 版）。初回は日付だけで書く旧規約が「初回と再走で命名系統が割れる」構造原因だったため、初回から時刻付きに寄せて系統を 1 本化する。
