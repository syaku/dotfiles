...
description: 新しく得た規範・知識・ユーザの人物像/好み・運用ノウハウを永続化する際、どの器（auto memory / paths なし rules / paths 付き rules / local-*.md / プロジェクト CLAUDE.md / skill）に置くかを判断するスキル。機密の隔離・paths glob の書き方・rules の入場条件・CLAUDE.md と rules の境界判定・モデル世代での再ベースラインも扱う。「これを覚えて」「ルールにして」「規範として残して」など永続化先の判断が要るとき、およびモデル系列を変えた直後の rules 見直しで参照する。
---
...
ファイル分離するが CLAUDE.md 本体の通読文脈に組み込みたいものは、CLAUDE.md 本体に直書きせず `~/.claude/<name>.md` に切り出して `@import` で取り込む。rules ディレクトリの自動ロード対象外で、rules の入場条件は適用しない。典型は基盤文脈（態度ペルソナ `persona.md`・ユーザプロフィール `user-profile.md` のように、主役を別ファイルに置き CLAUDE.md には導入の地の文だけを残すもの）と、常時厳守だがタスク文書でないもの（`knowledge-location.md` の業務機密隔離）。
...
なお、こうした規約定義・前提説明そのものは失敗訂正残差ではないので rules に置かない。この skill が正本を持つ。

## モデル世代での再ベースライン

rules は事前分布への残差であり、事前分布はモデル世代ごとに変わる。前世代の失敗を訂正したルールは、次世代では焼き込み済みの死荷重や過剰訂正になりうる。

- モデル系列を変えたら rules を一巡し、各ルールの錨の失敗が今のモデルでも起きるかを確認する。
- 再現しない・事前分布に焼き込まれたと判断したものは退役試行する（即削除でなく、コメントアウトや local 退避で一定期間観察してから消す）。
- ルールがあるのに違反が再発している場合は、強調の追加ではなく錨・適用条件の書き直しで直す（再発は書き方が届いていないシグナル）。
