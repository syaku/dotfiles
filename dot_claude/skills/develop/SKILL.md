...
- **(c) 3 ラウンド非収束時の報告** — develop 所有。台帳の review エントリが 3 件に達してなお指摘が残る場合、黙って打ち切らず残指摘をユーザに報告してから次へ進む。skill トラックの報告には**「非収束の変更が target に apply 済みのまま稼働している」事実と、source revert＋再 apply で巻き戻せること**を必ず含める（未収束の skill が現役で動き続けることをユーザから見える形にする）。この経路で step 5 へ進むときは台帳の `status` を `non-converged` にする（`converged` と区別する）。
...
- 軽微なクリーンアップが中心なら `simplify` スキルでも代替可（品質のみ・バグは見ない）。代替時も上記の修正 subagent と同じ必須項目（副作用禁止の明示・skill トラックの source 編集規約を含む）を伝搬させる。
...
- 修正後の客観確認はトラック表で引く（code: 台帳のテストコマンドを同一作業ツリーで再実行／skill: apply → diff 空 → code-review＋名前参照追跡）。**skill トラックの code-review 再実行は、下の収束ループの計器再実行と同一実行**（別に走らせない）。**1 ラウンドの連鎖単位を一意にする**: `apply → diff 空 → code-review → 名前参照追跡` の 1 連鎖が **1 ラウンド＝review エントリ 1 件**（名前参照追跡は本体 Read の独立ステップだが **code-review と同一ラウンドに含め、別ラウンドとして二重計数しない**。収束判定は「code-review 指摘ゼロ AND 名前参照追跡 pass」を 1 ラウンドの結果として台帳に記録し、3 ラウンド cap の起点を一意にする）。
