...
- `Skill` tool は main で動くため、隔離されるのは implement が起こす実装 subagent のコード編集工程（部分分離。詳細は implement 側「実行モデルと context 分離」節）。
...
- step 2 の implement 内 self-review とは観点で住み分ける（self-review＝plan 突合＋テスト green、ここ＝correctness/quality の adversarial レビュー。典拠は各 description）。同じバグ探しを二重にしない。
