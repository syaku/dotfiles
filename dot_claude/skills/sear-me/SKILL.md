---
name: sear-me
description: 計画に入る前に、隠れた前提・上流の目的・スコープ境界・成功条件・未決事項を素早く炙り出して ~/workspace/tasks/<タスクディレクトリ名>/premise.md に整理するスキル。draft-first 構成——まず premise.md の draft を書いて自己充足の仮定を可視化し、draft に残った分岐点だけを最大 2 ラウンドで質問して status: final に確定する。成果物は premise.md（status: final が plan への引き渡し条件）。grill-me の軽量版（炙る＝表面だけ素早く）で、芯まで焼く徹底尋問はしない。「前提を整理して」「計画前に炙り出して」「要件を固めて」などの依頼で起動する。
---

# sear-me: 前提を炙り出すフェーズ（plan の前段）

依頼を read-only で最小限調べ、まず premise.md の **draft を書き**、draft に残った分岐点だけをユーザに質問し、回答を反映した確定版（`status: final`）を書いて終了するスキル。plan の足場を作るのが役目で、自分で計画は立てず実装にも進まない（plan・implement の責務）。
...
- **Write は `~/workspace/tasks/<タスクディレクトリ名>/premise.md` のみ**（draft と確定版の 2 回が基本形）。他のファイルは書かない。
...
- タスクディレクトリ名は `~/workspace/tasks/CLAUDE.md` の命名規則に従う（内容が一目で分かる日本語名。既存の plan.md があるディレクトリなら同居させる）。
...
- **Goal / Done + Why**: 観測可能な完了条件＋**その上流の目的**（なぜこれをやるのか）。
