...
drain は**運用ログへの記録もカーソル更新もしない**（次回の日次 /harvest がカーソル経由で drain 産ノートを拾い直し、命名・層判定を opus で見直す二段構えの後段が担当）。
...
作成・fold・逆リンク・リンク張り替え・done 化・archive 退避（input/ 残量）・保留（理由付き）を箇条書きで報告する。運用ログ追記・カーソル更新は**しない**（日次 /harvest の担当）。
...
- 運用ログ・カーソルを drain で動かす（日次 /harvest の担当）
