...
prompt template と追加指示ブロックは **`~/.claude/skills/drain/references/prompts/insight-detect.md` を Read** して取得する（workflow `harvest-pipeline.js` の `insightPrompt` 本文の逐語転記＋追加指示ブロック。drain mode 用に backfillFocus 節は省略済み）。`<VAULT>` `<newNotesList>` `<extraMaterial>` `<NOW>` `<TODAY>` プレースホルダは spawn 時に値を埋める。追加指示ブロック（title_candidates 3〜4 案・form 必須・**self-check 手順＝反証点検**。判断基準 7 項目は `references/vault-rules.md`「洞察の命名 self-check 判断基準」節への Read 参照——2026-07-06 の backfill 同型化で workflow `insightPrompt` にも同型ブロックが載り、drain 固有ではなくなった）は spawn 時に prompt の末尾へ足す。差分管理は「workflow との interface」節を参照。
...
- self-check の判断基準は洞察用 7 項目（判断軸を名指す・失敗形不可 等。逐語の正本は `references/vault-rules.md`「洞察の命名 self-check 判断基準」節——insight-detect prompt と workflow backfill の `insightPrompt` が同節を Read 参照し、旧 `insightCriteria` との keep-in-sync 関係は 2026-07-06 に解消）。反証点検の突き合わせ先は `derivation.common_axis`（空のときのみ claim に退避）。
...
vault 規約・命名規約の prose は **`references/vault-rules.md` が単一の正本**（2026-07-06 に workflow 側 `VAULT_RULES` / `NAMING_*` 定数から移設）。workflow 側 prompt も `references/prompts/` 側 prompt もパス参照で agent 自身に Read させるため、規約 prose の keep-in-sync は不要になった。節名（「vault 規約」「命名規約 (kind 共通の核)」「気づきの命名」「洞察の命名」「洞察の命名 self-check 判断基準」）を変える場合だけ、全参照元（workflow `rulesRef` 呼び出し・`references/prompts/` の参照行）の節名指定を追従させる。**気づき命名基準**（旧 workflow noteCriteria・旧 kizuki-checker.md の 7 項目）は **`references/prompts/kizuki-extract.md` 内の self-check 節が唯一の正本**（R2-12 で workflow から noteCriteria を撤去・self-check 化で checker prompt ファイルも廃止し正本を extract prompt 内へ移した）——`references/prompts/` 側のみ変更する。洞察命名基準（self-check 判断基準 7 項目）は **`references/vault-rules.md`「洞察の命名 self-check 判断基準」節が唯一の正本**（2026-07-06 の backfill 同型化で workflow `insightCriteria` と洞察の checker/renamer を廃止し、`insight-detect.md` 内の逐語も同節への Read 参照に置換——判断基準の keep-in-sync 関係は解消）。なお `references/prompts/insight-detect.md` と workflow `insightPrompt` の逐語転記差分は、backfillFocus 節・mode 固有句と、**self-check 手順・title_candidates 指示の段落**（backfill 同型化で両側に載った。手順段落と要素 shape 例（string 配列で返す事故の防止・2026-07-05 drain 27 回目の shape 違反接地）は両 prompt に残る keep-in-sync 対象。shape 記述の粒度は揃えなくてよい——drain 側は prose で shape を要求（Agent tool に schema パラメータが無い）・workflow 側は `INSIGHT_SCHEMA` の構造化出力で型を強制する経路差があるため）——few-shot 参照句自体は両側同一に保つ。prompt 本体側の既知差分（意図的・逐語同期の対象外）: (a) 冒頭の書き込み禁止文言（drain 側「Write は呼び出し元の責務」・workflow 側「Read/Grep/Glob のみ」——agent の実ツール構成が違うため）、(b) MCP 不達 fallback の記述が drain 側は要約形（実 rg コマンドは workflow 側のみ）、(c) vault-rules.md への参照指示行の書式——workflow 側は `rulesRef` helper が VAULT/NOW/TODAY の値を interpolate して埋め込み・drain 側はプレースホルダの対応付けを prose で指示する（参照先の規約ファイルと節は同一）。common_axis の output-style 準拠指示は 2026-07-06 に workflow 側へも追従済みで既知差分から外れた。

**命名訂正事例集**: `~/.claude/skills/drain/naming-corrections.md`（chezmoi source: `dot_claude/skills/drain/naming-corrections.md`）が唯一の正本。参照方式・更新方式（パス参照のみ・静的キュレーション・NAMING 優先則）は同ファイル末尾の「keep-in-sync 方針」節を正本とする。SKILL 固有の受容事項: workflow `insightPrompt`（backfill で走る）から drain 内部パス（`naming-corrections.md`・`references/vault-rules.md`）へ依存が生じる——2026-07-06 の backfill 同型化で再検討し、規約 prose の共通正本を drain references/ に集約する配置として受容を維持。
