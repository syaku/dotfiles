...
各 kizuki-extract-N agent への prompt template は **`~/.claude/skills/drain/references/prompts/kizuki-extract.md` を Read** して取得する（逐語転記の管理点）。prompt には agent の責務限定・A 化命名ゲートとの interaction（SendMessage による再命名・軸再考）・derivation の 4 サブフィールド要件（`source_observations` / `pattern_generalization` / `lesson_axis` / `generalization_check`）・title_candidates の shape 指示（3 案固定・先頭が推奨案）・MCP 経路の使い方・命名訂正事例集の参照が含まれる。
...
2. **checker agent の呼び出し** (推奨案が regex 通過のときのみ・**適用は推奨案 = title_candidates 先頭に限定**——代替候補の品質は人ゲートが最終層として担う): 新規 `Agent` tool を spawn する（**name は `check-kizuki-<id>` のように別 name**・モデル sonnet・別 context）。prompt template は **`~/.claude/skills/drain/references/prompts/kizuki-checker.md` を Read** して取得する（判断基準 7 項目の逐語転記）。`<推奨案>` と `<derivation.lesson_axis (空なら source_excerpt) の逐語>` プレースホルダは spawn 時に埋める。
...
3. **SendMessage で再命名要請** (verdict が「該当」または step 1 で先頭（推奨案）に regex hit があった場合のみ——非先頭候補の hit は除外のみでラウンドを起動しない): 元の kizuki-extract-N agent に SendMessage を打ち、以下の renamer 指示を渡す。**この SendMessage 打鍵 1 回ごとに `sendmessage_invocations` を ++ する** (per_part_metrics.kizuki_insight.sendmessage_invocations の populate 経路・R2-13——明示 wire が無いと observed value が常時 0 になる)。**計上規則の正本は本 step**——step 5 トリアージ後の再命名・軸再考の打鍵も本カウンタに計上する（4.4・4.5・step 5 は本 wire への参照で畳む）。
   ...
   **`指摘:` 行の構築 (経路別)**:
   ...
   renamer prompt template は **`~/.claude/skills/drain/references/prompts/kizuki-renamer.md` を Read** して取得する（命名規約と気づき固有指示の逐語転記）。`<推奨案>` `<derivation.lesson_axis (空なら source_excerpt)>` と経路別の `<指摘>` プレースホルダは SendMessage 時に埋める。
...
prompt template と drain 固有の追加指示は **`~/.claude/skills/drain/references/prompts/insight-detect.md` を Read** して取得する（workflow `harvest-pipeline.js` の `insightPrompt` 本文の逐語転記＋drain 固有の title_candidates 追加指示。drain mode 用に backfillFocus 節は省略済み）。`<VAULT>` `<newNotesList>` `<extraMaterial>` プレースホルダは spawn 時に値を埋める。drain 固有の追加指示（title_candidates 3〜4 案・form 必須）は spawn 時に prompt の末尾へ足す。差分管理は「workflow との interface」節を参照。
...
##### checker prompt の本文 (洞察)

洞察 checker prompt template は **`~/.claude/skills/drain/references/prompts/insight-checker.md` を Read** して取得する（判断基準 7 項目の逐語転記）。`<推奨案>` と `<derivation.common_axis (空なら claim) の逐語>` プレースホルダは spawn 時に埋める。

構造化出力 schema: `{ verdict: ('該当'|'非該当'|'判断不能'), violations: [{criterion, quote, note}] }`

##### renamer prompt の本文 (洞察)

洞察 renamer prompt template は **`~/.claude/skills/drain/references/prompts/insight-renamer.md` を Read** して取得する（命名規約・洞察固有指示・title_candidates shape の逐語転記）。`<推奨案>` `<derivation.common_axis (空なら claim)>` と経路別の `<指摘>` プレースホルダは SendMessage 時に埋める。`指摘:` 行の構築は 4.2 と同じ経路別 fallback（regex hit 経路は「機械ゲート hit: <hits>」・checker 経路は violations 連結）。
...
- 別 context の sub-agent（schema = `FIX_SCHEMA`・model = sonnet）を spawn し、prompt template は **`~/.claude/skills/drain/references/prompts/fix.md` を Read** して取得する（workflow `fixAndRevalidate` 内本文の逐語転記）。`<errs.join(' / ')>` `<NOW>` `<TODAY>` `<c.content>` プレースホルダは spawn 時に埋める。
...
宛先は元の抽出 agent（kizuki-extract-N または insight-detect）——新規 agent で再導出しない（A 化原則）。打鍵計上は 4.2 step 3 に従う。prompt template は **`~/.claude/skills/drain/references/prompts/axis-redo.md` を Read** して取得する（軸再導出の逐語転記・返り値 shape 定義を含む）。`<気づき|洞察>` `<lesson_axis または common_axis の逐語>` `<title_candidates の一覧>` `<指摘の逐語>` プレースホルダは SendMessage 時に埋める。返り値 shape は `{ derivation, title_candidates, content }`。
...
Phase 4 で workflow から skill 本体側に移った責務（気づき抽出 prompt・洞察検出 prompt・A 化命名ゲート）の prompt 本文は **`~/.claude/skills/drain/references/prompts/*.md` に外出し**してある（skill 本体から workflow utility を import できないため。SKILL.md 本文はフロー骨格と shape 検査に絞り、逐語 prompt は references 側の管理点で保守する）。`references/prompts/` 配下の各ファイル対応:

- `kizuki-extract.md` — 気づき抽出 agent 用（step 4.1 が参照）
- `kizuki-checker.md` — 気づき命名 checker 用（step 4.2 step 2 が参照）
- `kizuki-renamer.md` — 気づき再命名 SendMessage 用（step 4.2 step 3 が参照）
- `insight-detect.md` — 洞察検出 agent 用（step 4.3 が参照・drain 固有の title_candidates 追加指示を含む）
- `insight-checker.md` — 洞察命名 checker 用（step 4.4 が参照）
- `insight-renamer.md` — 洞察再命名 SendMessage 用（step 4.4 が参照）
- `axis-redo.md` — 軸再考 SendMessage 用（step 5 が参照）
- `fix.md` — 規約違反修正 sub-agent 用（4.7 step 3 が参照）

workflow 側の `VAULT_RULES` / `NAMING_FOR_KIZUKI` / `NAMING_FOR_INSIGHT` / `checkerPrompt` / `renamePrompt` を変更した場合は `references/prompts/` 配下の対応ファイルも同期する（両者の prompt 本文は手で keep-in-sync する関係）。**気づき checker 基準** (旧 workflow noteCriteria) は **`references/prompts/kizuki-checker.md` が唯一の正本**（R2-12 で workflow からは noteCriteria を撤去・非対称ドリフトの温床を断った）——`references/prompts/` 側のみ変更する。洞察 checker 基準 (workflow insightCriteria) は `references/prompts/insight-checker.md` へ転記済みで keep-in-sync 関係を維持する（workflow 側にも稼働 path がある——backfill mode の洞察検出が workflow 内 nameGate を通る）。なお `references/prompts/insight-detect.md` と workflow `insightPrompt` の逐語転記差分は、backfillFocus 節・mode 固有句に加え drain 固有の title_candidates 追加指示（`insight-detect.md` 内に明示・spawn 時に末尾へ足す 1 段落）——few-shot 参照句自体は両側同一に保つ。**renamer の shape 差分（意図的・逐語同期の対象外）**: `references/prompts/kizuki-renamer.md` / `insight-renamer.md` は `{title_candidates}` 複数案を返す drain A 化ゲート専用の shape で、workflow の `renamePrompt` / `RENAME_SCHEMA` は `{title}` 単一のまま（backfill nameGate 用）。この差分は insightPrompt 差分と同じく明文管理された意図的差分で、逐語同期の対象にしない。
