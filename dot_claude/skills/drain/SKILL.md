...
- **家風タイトルの収集**: frontmatter tags に `気づき`/`洞察` を含む pages を `rg -l` で洗い、ファイル名（拡張子なし）の一覧を作る（多い場合は新しめ 20 件程度）。命名 agent が vault の確立した家風を実例として倣うための同梱素材（失敗接地: 2026-06-12 初回実走で命名 agent が素の文脈で命名し、説明文型の長いタイトルを量産した）。
