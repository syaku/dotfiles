...
2. 各候補について vault 既存ノードを突き合わせ、関連ノート・既出を洗う。一次索引は MCP tool 経由で動的に引く。
   - タイトル一致・意味近傍: **2 本のクエリを呼んで hits を合算して見る**——`mcp__vault-catalog__search_hybrid(query=候補タイトル, limit=12)` と `mcp__vault-catalog__search_hybrid(query=<素材の中心語: source_observations にある固有名詞・具体語をスペース区切りで 3〜6 語>, limit=12)`(一般化した候補タイトルは命名で具体語彙が剥がれて検索 anchor が弱いため、素材側の語彙で 2 本目を引く)。index は section 粒度で同一ノートが複数 hit を占めるため、hit のノート数は見かけより少ない前提で読む。
