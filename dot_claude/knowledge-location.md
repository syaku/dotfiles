...
- 強制力を要する行動規範は auto memory でなく rules に置く（recalled memory は context 扱いで強制力が弱い）。memory に残すのは事実・人物像・タスク状態と、観察記述として書いた feedback。規範体（〜するな／〜しろ）では書かない。書き口は [[agent-behavior]] 「auto memory の書き口」節を参照。
